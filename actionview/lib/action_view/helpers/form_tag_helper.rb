  ...
  module Helpers
    ...
    module FormTagHelper
      ...
      def select_tag(name, option_tags = nil, options = {})
        option_tags ||= StringPool::EMPTY
        html_name = (options[:multiple] == true && !name.to_s.ends_with?("[]")) ? "#{name}[]" : name

        if options.delete(:include_blank)
          option_tags = content_tag(:option, StringPool::EMPTY, :value => StringPool::EMPTY).safe_concat(option_tags)
        end
        ...
        content_tag :select, option_tags, { StringPool::NAME => html_name, StringPool::ID => sanitize_to_id(name) }.update(options.stringify_keys)
      end
      ...
      def text_field_tag(name, value = nil, options = {})
        tag :input, { StringPool::TYPE => "text", StringPool::NAME => name, StringPool::ID => sanitize_to_id(name), StringPool::VALUE => value }.update(options.stringify_keys)
      end
      ...
      def text_area_tag(name, content = nil, options = {})
        ...
        content_tag :textarea, content.to_s.html_safe, { StringPool::NAME => name, StringPool::ID => sanitize_to_id(name) }.update(options)
      end
      ...
      def submit_tag(value = "Save changes", options = {})
        ...
        tag :input, { StringPool::TYPE => "submit", StringPool::NAME => "commit", StringPool::VALUE => value }.update(options)
      end
      ...
      def button_tag(content_or_options = nil, options = nil, &block)
        ...
        options.reverse_merge! StringPool::NAME => 'button', StringPool::TYPE => 'submit'
      ...
      private
        ...
        def extra_tags_for_form(html_options)
          ...
          method_tag = case method
            when /^get$/i # must be case-insensitive, but can't use downcase as might be nil
              ...
              StringPool::EMPTY
            when /^post$/i, StringPool::EMPTY, nil
          ...
          tags = (enforce_utf8 ? utf8_enforcer_tag : StringPool::EMPTY.html_safe) << method_tag
        ...
        def sanitize_to_id(name)
          name.to_s.delete(']').gsub(/[^-a-zA-Z0-9:.]/, StringPool::UNDERSCORE)
        end
    end
  end
end
